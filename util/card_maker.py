...
from util.path_safety import sanitize_filename
...
def _create_base_canvas(
    ...
    draw.rectangle(
        [
            const.PIXELS_OUTER_MARGIN,
            const.PIXELS_OUTER_MARGIN,
            width - const.PIXELS_OUTER_MARGIN,
            height - const.PIXELS_OUTER_MARGIN,
        ],
...
def _draw_main_pinyin(
    ...
    draw.text(
        ((width - py_w) // 2, py_y), py_text, fill=const.COLOR_MAIN_PINYIN, font=font
    )
    return py_h


def _draw_main_character(
    draw: ImageDraw.ImageDraw,
    character: str,
    font: ImageFont.FreeTypeFont,
    width: int,
    ...
    draw.text(
        ((width - hz_w) // 2, hz_y),
        character,
        fill=const.COLOR_MAIN_CHARACTER,
        font=font,
    )


def _draw_bottom_phrases(
    draw: ImageDraw.ImageDraw,
    phrases: list[ChineseCharacterPhraseConfig] | None,
    font_text: ImageFont.FreeTypeFont,
    font_py: ImageFont.FreeTypeFont,
    width: int,
    height: int,
):
    ...
    draw.line(
        [
            const.PIXELS_DIVIDER_PADDING,
            bottom_boundary,
            width - const.PIXELS_DIVIDER_PADDING,
            bottom_boundary,
        ],
    ...
    for idx, pc in enumerate(phrases):
        ...
        pinyin_list = (
            pc.computed_character_phrase_pinyin
        )  # 列表, 已经转好的 ["zhōng", "guó"]
        ...
        total_word_width = (char_w * total_word_len) + (
            char_spacing * (total_word_len - 1)
        )
        ...
        sample_py_bbox = draw.textbbox(
            (0, 0), current_phrase_pinyin_sample, font=font_py
        )
        ...
        for i in range(total_word_len):
            ...
            # 计算当前汉字的核心中轴线 X 坐标
            current_char_center_x = (
                word_start_x + (i * (char_w + char_spacing)) + (char_w / 2)
            )
            ...
            py_w = py_bbox[2] - py_bbox[0]
            draw.text(
                (current_char_center_x - py_w // 2, py_pos_y),
                single_py,
                fill=const.COLOR_PHRASE_PINYIN,
                font=font_py,
            )

            # 2. 绘制对应的单字汉字（🔑 垂直使用统一算好的 word_pos_y，确保绝对水平对齐）
            draw.text(
                (current_char_center_x - char_w // 2, word_pos_y),
                single_char,
                fill=const.COLOR_PHRASE_TEXT,
                font=font_text,
            )
...
def draw_single_card(
    config: ChineseCharacterConfig,
    width: int = const.DEFAULT_CARD_WIDTH,
    ...
    try:
        font_text = ImageFont.truetype(
            const.CHARACTER_FONT, int(width * const.RATIO_FONT_SIZE_MAIN_CHARACTER)
        )
        font_pinyin = ImageFont.truetype(
            const.PINYIN_FONT, int(width * const.RATIO_FONT_SIZE_MAIN_PINYIN)
        )
        font_phrase = ImageFont.truetype(
            const.CHARACTER_FONT, int(width * const.RATIO_FONT_SIZE_PHRASE_TEXT)
        )
        font_phrase_py = ImageFont.truetype(
            const.PINYIN_FONT, int(width * const.RATIO_FONT_SIZE_PHRASE_PINYIN)
        )
    except IOError as e:
        raise e
    ...
    py_h = _draw_main_pinyin(
        draw, config.computed_character_pinyin, font_pinyin, width, height
    )

    _draw_main_character(draw, config.character, font_text, width, py_h, height)

    _draw_bottom_phrases(
        draw, config.character_phrases, font_phrase, font_phrase_py, width, height
    )

    return _save_card_image(image, config.character, output_path=output_path)


def _save_card_image(
    image: Image.Image, character: str, output_path: Path | None = None
) -> Path:
    ...
    if not output_path:
        ...
        if not target_dir.is_dir():
            raise NotADirectoryError(
                f"错误: 传入的路径 '{output_path}' 不是一个合法或已存在的目录。"
            )

    target_dir.mkdir(parents=True, exist_ok=True)

    safe_filename = sanitize_filename(character)
    if not safe_filename:
        safe_filename = "unknown"

    file_path = target_dir / f"{safe_filename}.png"
